...
DECLARE
  ...
  v_new_variant_name text := NULL;
  v_new_variant_color text := NULL;
  v_new_variant_size text := NULL;
  v_new_variant_sku_suffix text := NULL;
  v_new_variant_price numeric := NULL;
  v_snapshot jsonb;
  v_new_price numeric;
  v_new_cost numeric;
  v_sku text;
BEGIN
  ...
  -- Fetch new variant if provided (into scalar vars to avoid unassigned record error)
  IF p_new_variant_id IS NOT NULL THEN
    SELECT name, color, size, sku_suffix, price
      INTO v_new_variant_name, v_new_variant_color, v_new_variant_size, v_new_variant_sku_suffix, v_new_variant_price
      FROM product_variants WHERE id = p_new_variant_id;
  END IF;

  -- Determine new pricing
  v_new_price := COALESCE(v_new_variant_price, v_new_product.price, 0);
  v_new_cost := public.get_product_effective_cost(p_new_product_id, p_new_variant_id);
  v_sku := CASE
    WHEN v_new_variant_sku_suffix IS NOT NULL THEN COALESCE(v_new_product.sku,'') || '-' || v_new_variant_sku_suffix
  ...
  UPDATE order_items SET
    ...
    sku_snapshot = v_sku,
    variant_name_snapshot = v_new_variant_name,
    color_snapshot = v_new_variant_color,
    size_snapshot = v_new_variant_size
