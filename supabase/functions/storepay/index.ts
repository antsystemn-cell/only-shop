...
async function finalizePayment(supabase: any, pi: any) {
  ...
  if (pi.type === "order") {
    ...
    if (order) {
      ...
      if (otOrder) {
        console.log("[storepay] OT order updated:", otOrder.order_number);
        notifyAdminPayment(supabase, otOrder.order_number, otOrder.subtotal, "Storepay").catch(console.error);
      } else {
        console.error("[storepay] Order not found in either table:", pi.reference_id);
      }
    }
  } else if (pi.type === "wallet_topup") {
