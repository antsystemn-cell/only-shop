        ...
        <div className="flex items-center gap-2 md:gap-4 overflow-x-auto scrollbar-hide px-3 md:px-0 pb-1 border-black">
          {categories.map(category => <Link key={category.id} to={`/shop?category=${category.id}`} className="group flex flex-col items-center gap-1.5 md:gap-2 min-w-[60px] md:min-w-[90px] shrink-0">
              ...
              <div className="w-12 h-12 md:w-16 md:h-16 rounded-full md:rounded-lg overflow-hidden bg-white/10 flex items-center justify-center group-hover:bg-white/20 group-active:bg-white/25 transition-colors ring-2 ring-transparent group-hover:ring-white/20">
                {category.image_url ? <img src={category.image_url} alt={category.name_mn} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300 border-black/0 border-none border-0" /> : <Folder className="h-5 w-5 md:h-7 md:w-7 text-white/70" />}
              </div>
