    ...
    mutationFn: async (asin: string) => {
      ...
      const { error } = await supabase.functions.invoke("amazon-api", {
        body: {
          ...
          params: { asins: [asin], marketplaceId: product?.marketplace_id },
        },
      });
      if (error) throw error;
    },
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Бараанууд ({filtered?.length || 0})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex gap-4 mb-4">
            <Input
              placeholder="Нэр, ASIN-аар хайх..."
              value={search}
                  ...
                  <TableCell>
                    <p className="text-xs text-muted-foreground">
                      {product.last_synced_at
                        ? new Date(product.last_synced_at).toLocaleString("mn-MN")
                        : "—"}
                    </p>
                  </TableCell>
